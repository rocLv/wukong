# How to provision a new environment for Wukong
...
* Enter domains to secure (for example): `wukong.com`
* Enter webroot path: `/home/deployer/apps/Wukong/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/wukong.com/fullchain.pem /etc/ssl/Wukong.crt
ln -s /etc/letsencrypt/live/wukong.com/privkey.pem /etc/ssl/Wukong.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/Wukong/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
14 10,22 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/Wukong/shared/public/
```
